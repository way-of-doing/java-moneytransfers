...
public class MoneyTransferSerializer implements JsonDeserializer<MoneyTransfer> {
    ...
    public MoneyTransfer deserialize(JsonElement element, Type type, JsonDeserializationContext context) throws JsonParseException {
        ...
        if (from == null || !from.isJsonPrimitive()) {
            throw new JsonDeserializationException("money transfer 'from' element missing or not a string");
        } else if (to == null || !to.isJsonPrimitive()) {
            throw new JsonDeserializationException("money transfer 'to' element missing or not a string");
        } else if (value == null || !value.isJsonObject()) {
            throw new JsonDeserializationException("money transfer 'value' element missing or not an object");
        }
        ...
        if (valueCurrency == null || !valueCurrency.isJsonPrimitive()) {
            throw new JsonDeserializationException("money transfer value 'currency' element missing or not a primitive");
        } else if (valueAmount == null || !valueAmount.isJsonPrimitive()) {
            throw new JsonDeserializationException("money transfer value 'amount' element missing or not a primitive");
        }
